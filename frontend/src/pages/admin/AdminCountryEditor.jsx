...
export default function AdminCountryEditor() {
  ...
  const handleAddGalleryImage = async (file) => {
    ...
    try {
      const res = await uploadImage(file, 'wandersync/countries');
      if (res.data?.url) {
        setFormData((prev) => ({ ...prev, images: [...prev.images, res.data.url] }));
        showToast('Scenic photo added to country gallery', 'success');
      }
    } catch {
            ...
            <div className="md:col-span-8 space-y-3">
              <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
                <div className="space-y-1">
                  <label className="text-[11px] font-bold text-zinc-300">Country Name *</label>
                  <input
                    type="text"
                    required
                    value={formData.name}
                ...
                <div className="space-y-1">
                  <label className="text-[11px] font-bold text-zinc-300">ISO Code</label>
                  <input
                    type="text"
                    value={formData.code}
                    onChange={(e) => setFormData({ ...formData, code: e.target.value.toUpperCase() })}
                    placeholder="e.g. JP, IT, FR"
                    className="w-full px-3 py-1.5 rounded-lg bg-secondary/60 border border-border text-xs text-foreground uppercase focus:outline-none focus:ring-1 focus:ring-orange-500/50 font-mono"
                  />
                </div>
          ...
          <div className="pt-3 border-t border-border/70 space-y-2">
            <div className="flex items-center justify-between">
              ...
              <label className="text-[11px] font-bold text-orange-400 hover:text-orange-300 flex items-center gap-1 cursor-pointer">
                ...
                <span>{uploadingGallery ? 'Uploading...' : 'Add Scenic Photo'}</span>
                <input
                  type="file"
                  accept="image/*"
