const REGISTRY = {
  saudi: {
    keys: ['saudi', 'ksa', 'riyadh', 'jeddah', 'mecca', 'medina', 'alula', 'diriyah'],
    cover: 'https://images.unsplash.com/photo-1586724237569-f3d0c1dee8c6?w=1200&auto=format&fit=crop&q=80',
    gallery: [
  ...
  uae: {
    keys: ['uae', 'emirates', 'dubai', 'abu dhabi', 'sharjah', 'burj'],
  ...
  pakistan: {
    keys: ['pakistan', 'pak', 'pk', 'islamabad', 'lahore', 'karachi', 'hunza', 'skardu', 'murree'],
  ...
  japan: {
    keys: ['japan', 'jpn', 'tokyo', 'kyoto', 'osaka', 'sapporo', 'fuji', 'shibuya'],
  ...
  switzerland: {
    keys: ['switzerland', 'swiss', 'zurich', 'geneva', 'lucerne', 'zermatt', 'alps', 'matterhorn'],
  ...
  france: {
    keys: ['france', 'paris', 'nice', 'lyon', 'eiffel', 'louvre'],
  ...
  italy: {
    keys: ['italy', 'rome', 'florence', 'venice', 'colosseum', 'milan'],
  ...
  turkey: {
    keys: ['turkey', 'turkiye', 'türkiye', 'istanbul', 'cappadocia', 'antalya', 'bosphorus'],
    cover: 'https://images.unsplash.com/photo-1524231757912-21f4fe3a7200?w=1200&auto=format&fit=crop&q=80',
    gallery: [
  ...
  uk: {
    keys: ['united kingdom', 'uk', 'london', 'edinburgh', 'england', 'scotland', 'britain'],
    cover: 'https://images.unsplash.com/photo-1513635269975-59663e0ac1ad?w=1200&auto=format&fit=crop&q=80',
    gallery: [
  ...
  usa: {
    keys: ['united states', 'usa', 'america', 'new york', 'los angeles', 'california'],
    cover: 'https://images.unsplash.com/photo-1496442226666-8d4d0e62e6e9?w=1200&auto=format&fit=crop&q=80',
    gallery: [
  ...
  spain: {
    keys: ['spain', 'barcelona', 'madrid', 'seville', 'valencia', 'ibiza'],
    cover: 'https://images.unsplash.com/photo-1543783207-ec64e4d95325?w=1200&auto=format&fit=crop&q=80',
    gallery: [
...
export const resolveEntityImages = (type, query = '', country = '', city = '') => {
  const text = `${country} ${city} ${query}`.toLowerCase();
  for (const pack of Object.values(REGISTRY)) {
    if (pack.keys.some((k) => text.includes(k))) {
      return { coverImage: pack.cover, images: pack.gallery };
    }
  }
  return { coverImage: REGISTRY.pakistan.cover, images: REGISTRY.pakistan.gallery };
};

export const resolveCityImage = (cityName = '', countryName = '') => {
  const text = `${countryName} ${cityName}`.toLowerCase();
  for (const pack of Object.values(REGISTRY)) {
    if (pack.keys.some((k) => text.includes(k))) {
      return pack.gallery[0] || pack.cover;
    }
  }
  return REGISTRY.pakistan.gallery[0];
};
